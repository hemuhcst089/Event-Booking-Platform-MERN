...
const { sendBookingEmail, sendOTPEmail, sendBookingRequestEmail } = require('../utils/email');

exports.bookEvent = async (req, res) => {
    try {
        const { eventId, seats } = req.body;
        const seatsToBook = seats !== undefined ? parseInt(seats, 10) : 1;

        const event = await Event.findById(eventId);
        if (!event) return res.status(404).json({ message: 'Event not found' });
        if (event.availableSeats <= 0) return res.status(400).json({ message: 'No seats available' });
        if (seatsToBook > event.availableSeats) return res.status(400).json({ message: 'Seats cannot exceed available seats' });
        ...
        const booking = await Booking.create({
            ...
            amount: event.ticketPrice * seatsToBook
        });

        await sendBookingRequestEmail(req.user.email, req.user.name, event.title);
...
exports.confirmBooking = async (req, res) => {
    try {
        ...
        event.availableSeats -= seatsToDeduct;
        await event.save();
    ...
    try {
        ...
        if (wasConfirmed) {
            ...
            if (event) {
                ...
                event.availableSeats += seatsToRestore;
                await event.save();
            }
        }
